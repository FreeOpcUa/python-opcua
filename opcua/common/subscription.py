...
class Subscription(object):
    ...
    def __init__(self, session_server, params, handler):
        ...
        response = self.session_server.create_subscription(params, self.publish_callback)
        ...
        self.session_server.publish()

    def delete(self):
        """
        Delete subscription on server. This is automatically done by Client and Server classes on exit
        """
        results = self.session_server.delete_subscriptions([self.subscription_id])
        results[0].check()
    ...
    def publish_callback(self, publishresult):
        ...
        self.session_server.publish([ack])
    ...
    def subscribe_events(self, sourcenode=ua.ObjectIds.Server, evtypes=ua.ObjectIds.BaseEventType, evfilter=None, queuesize=0):
        ...
        Return a handle which can be used to unsubscribe
        """
        sourcenode = Node(self.session_server, sourcenode)
    ...
    def create_monitored_items(self, monitored_items):
        ...
        with self._lock:
            for mi in monitored_items:
                ...
                data.node = Node(self.session_server, mi.ItemToMonitor.NodeId)
        ...
        results = self.session_server.create_monitored_items(params)
        mids = []
    ...
    def unsubscribe(self, handle):
        ...
        results = self.session_server.delete_monitored_items(params)
        results[0].check()
        with self._lock:
    ...
    def modify_monitored_item(self, handle, new_samp_time, new_queuesize=0, mod_filter_val=-1):
        ...
        results = self.session_server.modify_monitored_items(params)
