...
def _lsprint_0(node, depth, indent=""):
    ...
    for desc in node.get_children_descriptions():
        ...
        if depth:
            _lsprint_0(Node(node.isession, desc.NodeId), depth - 1, indent + "  ")
...
def _lsprint_1(node, depth, indent=""):
    ...
    for desc in node.get_children_descriptions():
        if desc.NodeClass == ua.NodeClass.Variable:
            try:
                val = Node(node.isession, desc.NodeId).get_value()
        ...
        if depth:
            _lsprint_1(Node(node.isession, desc.NodeId), depth - 1, indent + "  ")
