...
async def test_delete_references(opc):
    ...
    # clean-up
    await opc.delete_nodes([fold, newtype], recursive=True)
...
async def test_use_namespace(opc):
    idx = await opc.get_namespace_index("urn:freeopcua:python:server")
    assert 1 == idx
    root = opc.get_root_node()
...
async def test_method_none(opc):
    ...
    o = await opc.get_node(ua.ObjectIds.BaseObjectType).get_child("2:ObjectWithMethodsType")
...
async def test_incl_subtypes(opc):
    base_type = await opc.get_root_node().get_child(["0:Types", "0:ObjectTypes", "0:BaseObjectType"])
...
async def test_path_string(opc):
    o = await (await opc.nodes.objects.add_folder(1, "titif")).add_object(3, "opath")
...
async def test_path(opc):
    of = await opc.nodes.objects.add_folder(1, "titif")
    ...
    assert [
               opc.nodes.root, opc.nodes.types, opc.nodes.object_types, opc.nodes.base_object_type,
               opc.nodes.folder_type, opc.get_node(ua.ObjectIds.FileDirectoryType), target
           ] == path


async def test_get_endpoints(opc):
    endpoints = await opc.get_endpoints()
...
async def test_instantiate_1(opc):
    # Create device type
    dev_t = await opc.nodes.base_object_type.add_object_type(0, "MyDevice")
    v_t = await dev_t.add_variable(0, "sensor", 1.0)
    ...
    # instanciate device
    nodes = await instantiate(opc.nodes.objects, dev_t, bname="2:Device0001")
    mydevice = nodes[0]
    ...
    nodes = await instantiate(opc.nodes.objects, devd_t, bname="2:Device0002")
...
async def test_instantiate_string_nodeid(opc):
    # Create device type
    dev_t = await opc.nodes.base_object_type.add_object_type(0, "MyDevice2")
    v_t = await dev_t.add_variable(0, "sensor", 1.0)
    await v_t.set_modelling_rule(True)
    p_t = await dev_t.add_property(0, "sensor_id", "0340")
    await p_t.set_modelling_rule(True)
    ...
    # instanciate device
    nodes = await instantiate(opc.nodes.objects, dev_t, nodeid=ua.NodeId("InstDevice", 2, ua.NodeIdType.String),
        bname="2:InstDevice")
    mydevice = nodes[0]
...
async def test_variable_with_datatype(opc):
    v1 = await opc.nodes.objects.add_variable(
    ...
    v2 = await opc.nodes.objects.add_variable(
...
async def test_enum(opc):
    # create enum type
    enums = await opc.get_root_node().get_child(["0:Types", "0:DataTypes", "0:BaseDataType", "0:Enumeration"])
...
async def test_base_data_type(opc):
    ...
    ext = await opc.nodes.objects.add_variable(0, "MyExtensionObject", ua.Argument())
    d = await ext.get_data_type()
    d = opc.get_node(d)
    assert opc.get_node(ua.ObjectIds.Structure) == await ua_utils.get_base_data_type(d)
...
async def test_data_type_to_variant_type(opc):
    ...
    for dt, vdt in test_data.items():
        assert vdt == await ua_utils.data_type_to_variant_type(opc.get_node(ua.NodeId(dt)))
